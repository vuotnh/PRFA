...
from utils import l2_proj_maker, linf_proj_maker
import bbox_utils as demo_utils
...
class BlackBoxAttack(object):
    ...
    def summary(self):
        ...
        return {
            ...
            "average_num_queries": "NaN" if self.total_successes == 0 else (
                                                                                   self.total_loss_queries + self.total_crit_queries) / self.total_successes,
    ...
    def run(self, data, loss_fct, early_stop_crit_fct, vis_attack_step=None, log=None, results_records=None,
            quires_records=None, gt_info=None):
        """
        ...
        while True:
            ...
            # if its % 1 == 0: success_mask = dones_mask * correct_classified_mask total_successes = float(
            # success_mask.sum()) # import pdb; pdb.set_trace() print ("Iteration : ", its, 'ave_loss_queries : ',
            # (num_loss_queries * success_mask).sum() / total_successes, \ "ave_crit_queries : ", (num_crit_queries *
            # success_mask).sum()  / total_successes, \ "ave_queries : ", (num_loss_queries * success_mask).sum() /
            # total_successes + (num_crit_queries * success_mask).sum()  / total_successes, \ "successes : ",
            # success_mask.sum() / float(success_mask.shape[0]), \ "failures : ", (np.logical_not(dones_mask) *
            # correct_classified_mask).sum()  / float(success_mask.shape[0])) sys.stdout.flush()
