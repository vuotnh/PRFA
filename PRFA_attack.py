...
from utils import lp_step
from bbox_utils import bbox_to_attack_points, mask_to_attack_points, unique_rows, reppoints_to_attack_points
...
class IoUSSAttack(BlackBoxAttack):
    ...
    # # prevent trying out a delta if it doesn't change x_curr (e.g. an overlapping patch) if not self.flip_flag:
    # while np.sum(np.abs(np.clip(x_window + deltas[i_img, :, center_h:center_h+s, center_w:center_w+s], self.lb,
    # self.ub) - x_best_window) < 10**-7) == c*s*s: deltas[i_img, :, center_h:center_h+s, center_w:center_w+s] =
    # np.random.choice([-self.epsilon, self.epsilon], size=[c, 1, 1]) self.flip_deltas = deltas self.flip_flag = True
    # self.flip_zone = 'w1' else: # flip twice on w if self.flip_zone == 'w1': flip_deltas_sign = np.ones(size=[1, c,
    # s, s], dtype='float32') flip_deltas_sign[:, :, :, :flip_w_size] = flip_deltas_sign[:, :, :, :flip_w_size] *
    # -1.0 deltas[i_img, :, center_h:center_h+s, center_w:center_w+s] = self.flip_deltas[i_img, :,
    # center_h:center_h+s, center_w:center_w+s] * flip_deltas_sign self.flip_zone = 'w2' elif self.flip_zone == 'w2':
    # flip_deltas_sign = np.ones(self.flip_deltas.shape, dtype='float32') flip_deltas_sign[:, :, :flip_w_size:] =
    # flip_deltas_sign[:, :, :flip_w_size] * -1.0 deltas[i_img, :, center_h:center_h+s, center_w:center_w+s] =
    # self.flip_deltas[i_img, :, center_h:center_h+s, center_w:center_w+s] * flip_deltas_sign self.flip_zone = 'h1' #
    # self.flip_flag = False elif self.flip_zone == 'h1': flip_deltas_sign = np.ones(self.)
        ...
        if it in [0, 11, 51, 201, 501, 1001, 2001, 4001, 6001, 8001, 10001]:
            for patch_attack in patch_attacks.split(','):
                if patch_attack == 'bbox':
                    ...
                    data = {'img': [ori_img], 'img_metas': img_metas}
                    with torch.no_grad():
        ...
        if self.p == 'inf':
            if self.is_new_batch:
                ...
                if self.square_init is None or self.square_init == 'False':
                    ...
                    s = min(max(s, 1), h - 1)
                    if self.patch_attack is not None:
                        attack_points = self.attack_points_selection(self.patch_attack, self.i, h, w, clean_info,
                                                                     img_metas, s=s, ori_img=self.ori_img)
                    else:
                        attack_points = []
                    ...
                    for _ in range(0, int(self.attack_parallel)):
                        ...
                        xs[xs.shape[0] - 1, :, center_h_init:center_h_init + s, center_w_init:center_w_init + s] = xs[
                                                                                                                   xs.shape[
                                                                                                                       0] - 1,
                                                                                                                   :,
                                                                                                                   center_h_init:center_h_init + s,
                                                                                                                   center_w_init:center_w_init + s] + init_delta
            ...
            for i_img in range(xs.shape[0]):
                ...
                if self.patch_attack is not None:
                    attack_points = self.attack_points_selection(self.patch_attack, self.i, h, w, clean_info, img_metas,
                                                                 s=s, ori_img=self.ori_img)
                else:
                    attack_points = []
                ...
                for count, (center_h, center_w) in enumerate(zip(center_hs, center_ws)):
                    ...
                    if not self.flip_flag:
                        while np.sum(np.abs(np.clip(
                                x_window + deltas[i_img, :, center_h:center_h + s, center_w:center_w + s], self.lb,
                                self.ub) - x_best_window) < 10 ** -7) == c * s * s:
                            deltas[i_img, :, center_h:center_h + s, center_w:center_w + s] = np.random.choice(
                                [-self.epsilon, self.epsilon], size=[c, 1, 1])
                        ...
                        if self.flip_zone == 'h1':
                            flip_deltas_sign = np.ones((1, c, s, s), dtype='float32')
                            flip_deltas_sign[:, :, flip_size:, :] = flip_deltas_sign[:, :, flip_size:, :] * -1.0
                            deltas[i_img, :, center_h:center_h + s, center_w:center_w + s] = self.flip_deltas[i_img, :,
                                                                                             center_h:center_h + s,
                                                                                             center_w:center_w + s] * flip_deltas_sign
                            if count + 1 == attack_parallel_num:
                                self.flip_zone = 'h2'
                ...
                # # prevent trying out a delta if it doesn't change x_curr (e.g. an overlapping patch) if not
                # self.flip_flag: while np.sum(np.abs(np.clip(x_window + deltas[i_img, :, center_h:center_h+s,
                # center_w:center_w+s], self.lb, self.ub) - x_best_window) < 10**-7) == c*s*s: deltas[i_img, :,
                # center_h:center_h+s, center_w:center_w+s] = np.random.choice([-self.epsilon, self.epsilon],
                # size=[c, 1, 1]) self.flip_deltas = deltas[i_img, :, center_h:center_h+s, center_w:center_w+s]
                # self.flip_flag = True self.flip_zone = 'w1' else: # flip twice on w flip_w_size = int(
                # self.flip_deltas.shape[2] / 2) if self.flip_zone == 'w1': flip_deltas_sign = np.ones(
                # self.flip_deltas.shape, dtype='float32') flip_deltas_sign[:, :, :flip_w_size] = flip_deltas_sign[:,
                # :, :flip_w_size] * -1.0 deltas[i_img, :, center_h:center_h+s, center_w:center_w+s] =
                # self.flip_deltas * flip_deltas_sign self.flip_zone = 'w2' elif self.flip_zone == 'w2':
                # flip_deltas_sign = np.ones(self.flip_deltas.shape, dtype='float32') flip_deltas_sign[:, :,
                # flip_w_size:] = flip_deltas_sign[:, :, flip_w_size:] * -1.0 deltas[i_img, :, center_h:center_h+s,
                # center_w:center_w+s] = self.flip_deltas * flip_deltas_sign self.flip_flag = False
