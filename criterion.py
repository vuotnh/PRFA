...
def early_stop_crit_fct_with_iou(attacker, xs, img_metas, clean_info):
    ...
    # iou<0.3 or scores<0.05 correct = list() for object_clean in objects_clean: pred_indexes = np.where(
    # label_results==object_clean)[0] gt_indexes = np.where(labels_clean==object_clean)[0] for pred_index in
    # pred_indexes: flag = False for gt_index in gt_indexes: # if (compute_iou(bbox_results[int(pred_index)],
    # bboxes_clean[int(gt_index)]) > 0.5) and (score_results[int(pred_index)][object_clean] > 0.05): if (
    # score_results[int(pred_index)][object_clean] > 0.05) and (compute_iou(bbox_results[int(pred_index)],
    # bboxes_clean[int(gt_index)]) > 0.5): flag = True correct.append(flag) correct = np.array(correct)
    ...
    for object_clean in objects_clean:
        ...
        gt_indexes = np.where(labels_clean == object_clean)[0]
        # for pred_index in pred_indexes: flag = False for gt_index in gt_indexes: # if (compute_iou(bbox_results[
        # int(pred_index)], bboxes_clean[int(gt_index)]) > 0.5) and (score_results[int(pred_index)][object_clean] >
        # 0.05): if (score_results[int(pred_index)][object_clean] > 0.05) and (compute_iou(bbox_results[int(
        # pred_index)], bboxes_clean[int(gt_index)]) > 0.5): flag = True correct.append(flag)
